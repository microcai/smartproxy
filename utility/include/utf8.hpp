...
// utf8 to GBK
inline std::string utf8_gbk(const std::string& source);
inline std::string gbk_utf8(const std::string& source);
inline std::string utf8_gbk(const std::string& source)
{
	try {
		return boost::locale::conv::from_utf<std::string::value_type>(source, "GBK");
	}
	catch (...) {
		return {};
	}
...
{
	try {
		return boost::locale::conv::to_utf<std::string::value_type>(source, "GBK");
	}
	catch (...) {
		return {};
	}
